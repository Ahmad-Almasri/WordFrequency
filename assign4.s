...
argv:
	...
	bl	atoi				// call atoi
	str	w0, [x29, col]			// store col #

	//      ===================================================

	//	check # of row
	ldr	w19, [x29, row]			// get row
	...
	ldr	w1, [x29, row]			// get row
	ldr	w2, [x29, col]			// get col
...
allocation:
	...
	sub	x20, xzr, x20			// -(x20)
	str	x20, [x29, struct]		// store the offset

	//	allocate array of struct ... row * col(2) * sizeOfInt(4)
	ldr	w19, [x29, row]			// load # of row
	lsl	w19, w19, 1			// row * 2
...
//---------------------------------------------------------------
	...
	mov	w19, 0
	str	w19, [x29, i]			// update i
	str	w19, [x29, j]			// update j
...
fill:
	...
	str	w20, [x29, x19]	         	// store w20 at x29+x19-4
...
stopFill:
	mov	w19, 0				// set w19 = 0
	str	w19, [x29, j]			// update j = 0

	ldr	w19, [x29, i]			// get i
	add	w19, w19, 1			// i += 1
	str	w19, [x29, i]			// update i

	ldr	w20, [x29, row]			// get row
...
max:
	...
	ldr	w19, [x29, x19]			// get array[i][j]

	//      ===================================================
	//	Calc. The size of the document

	ldr	w20, [x29, sum]			// get sum
	add	w20, w19, w20			// sum+=array[i][j]
	str	w20, [x29,sum]			// update sum
...
strValue:
	...
	str	w19, [x29, maxVal]		// maxVal = array[i][j]
	
	//	store the Index [j] at index
	ldr	w19, [x29, j]
	str	w19, [x29, index]		// index = j
...
stopMax:
	...
	lsl	w19, w19, 3			// i * 8
	sub	x19, xzr, x19			// x19 *= -1
	ldr	x20, [x29, struct]		// get struct offset
	add	x19, x19, x20			// struct[i] offset

	//	store the Index in the struct[i].index
	ldr	w20, [x29, index]		// get index
	str	w20, [x29, x19]			// struct[i].index = index

	//	store the Frequency in the struct[i].freq
	ldr	w20, [x29, sum]			// get sum
	ldr	w21, [x29, maxVal]		// get maxVal
	mov	w22, 100			// w22 = 100
	mul	w21, w21, w22		// w21 = maxValue * 100
	udiv	w20, w21, w20			// freq = maxVal/sum

	add	x19, x19, -4			// update offset of struct by -4
	str	w20,[x29, x19]			// struct[i].freq = freq

	//      ===================================================

        mov     w19, 0
        str     w19, [x29, j]                    // update j = 0
	str	w19, [x29, sum]			// updata sum = 0
...
displayResult:
	...
	lsl	w19, w19, 3			// i * 8
	sub	x19, xzr, x19			// x19 *= -1
	ldr	x20, [x29, struct]	        // get struct
	add	x19, x19, x20			// struct[i] offset

	//      ===================================================

	//	Get The index & Frequency
	ldr	w21, [x29, x19]		// get Index
	add	x19, x19, -4			// update offset by -4 
	ldr	w22, [x29, x19]		// get freq.
	...
	//	Get The Occurrences ... using the stored index in the struct
        ...
        ldr     w20, [x29, col]                  // get col
        mul     w19, w19, w20                   // i * col
        add     w19, w19, w21                 // (i*col) + j
        lsl     w19, w19, 2                     // [i*col+j]*4
        add     w19, w19, 4                     // offset + 4
        sub     x19, xzr, x19                   // x19 *= -1

	ldr	w19, [x29, x19]			// get Occurrences
...
stopDy:
	ldr	x19, [x29, size]			// load the size
	sub	x19, xzr, x19			// -(size)
	add	sp, sp, x19			// update sp
stop:
	ldp	x29, x30, [sp], dealloc		// restore x29, x30
	ret					// stop the prog.
